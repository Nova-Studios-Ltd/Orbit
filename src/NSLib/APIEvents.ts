...
import { Manager } from "../Pages/AuthHandler";
...
export function UPDATEUsername(user_uuid: string, newUsername: string, callback: (status: boolean, newUsername: string) => void) {
    PATCH(`/User/${user_uuid}/Username`, ContentType.JSON, newUsername, Manager.User.token).then((resp: NCAPIResponse) => {
...
export function UPDATEPassword(user_uuid: string, newPassword: string, callback: (status: boolean, newPassword: string) => void) {
    PATCH(`/User/${user_uuid}/Password`, ContentType.JSON, newPassword, Manager.User.token).then((resp: NCAPIResponse) => {
...
export function UPDATEEmail(user_uuid: string, newEmail: string, callback: (status: boolean, newEmail: string) => void) {
    PATCH(`/User/${user_uuid}/Email`, ContentType.JSON, newEmail, Manager.User.token).then((resp: NCAPIResponse) => {
...
export function DELETEUser(user_uuid: string, callback: (status: boolean) => void) {
    DELETE(`/User/${user_uuid}`, Manager.User.token).then((resp: NCAPIResponse) => {
...
export async function GETKey(user_uuid: string, key_user_uuid: string) : Promise<string | undefined> {
    const resp = await GET(`/User/${user_uuid}/Keystore/${key_user_uuid}`, Manager.User.token);
...
export async function GETKeystore(user_uuid: string) : Promise<Dictionary<string> | undefined> {
    const resp = await GET(`/User/${user_uuid}/Keystore`, Manager.User.token);
    if (resp.status === 200) {
...
export async function SETKey(user_uuid: string, key_user_uuid: string, key: string) : Promise<boolean> {
    const resp = await POST(`/User/${user_uuid}/Keystore/${key_user_uuid}`, ContentType.JSON, key, Manager.User.token);
...
export function CREATEChannel(recipient_uuid: string, callback: (created: boolean) => void) {
    POST(`Channel/CreateChannel?recipient_uuid=${recipient_uuid}`, ContentType.EMPTY, "", Manager.User.token).then((resp: NCAPIResponse) => {
        if (resp.status === 200) callback(true);
        else callback(false);
    });
}
...
export function CREATEGroupChannel(group_name: string, recipients: string[], callback: (created: boolean) => void) {
    POST(`Channel/CreateGroupChannel?group_name=${group_name}`, ContentType.JSON, JSON.stringify(recipients), Manager.User.token).then((resp: NCAPIResponse) => {
        if (resp.status === 200) callback(true);
        else callback(false);
    });
}
...
export function UPDATEChannelName(channel_uuid: string, newName: string, callback: (updated: boolean) => void)  {
    PATCH(`/Channel/${channel_uuid}/Name?new_name=${newName}`, ContentType.EMPTY, "", Manager.User.token).then((resp: NCAPIResponse) => {
        if (resp.status === 200) callback(true);
        else callback(false);
    });
}
...
export function UPDATEChannelIcon(channel_uuid: string, file: Blob, callback: (updated: boolean) => void) {
    POSTFile(`/Media/Channel/${channel_uuid}/Icon`, file, Manager.User.token).then((resp: NCAPIResponse) => {
        if (resp.status === 200) callback(true);
        else callback(false);
    });
}
...
export function ADDChannelMember(channel_uuid: string, recipients: string[], callback: (added: boolean) => void) {
    PATCH(`Channel/${channel_uuid}`, ContentType.JSON, JSON.stringify(recipients), Manager.User.token).then((resp: NCAPIResponse) => {
        if (resp.status === 200) callback(true);
        else callback(false);
    });
}
...
export function REMOVEChannelMember(channel_uuid: string, recipient: string, callback: (removed: boolean) => void) {
    DELETE(`/Channel/${channel_uuid}/Members?recipient=${recipient}`, Manager.User.token).then((resp: NCAPIResponse) => {
        if (resp.status === 200) callback(true);
        else callback(false);
    });
}
...
export function ARCHIVEChannel(channel_uuid: string, callback: (archived: boolean) => void) {
    PATCH(`/Channel/${channel_uuid}/Achrive`, ContentType.EMPTY, "", Manager.User.token).then((resp: NCAPIResponse) => {
        if (resp.status === 200) callback(true);
        else callback(false);
    });
}
...
export function UNARCHIVEChannel(channel_uuid: string, callback: (archived: boolean) => void) {
    PATCH(`/Channel/${channel_uuid}/Unachrive`, ContentType.EMPTY, "", Manager.User.token).then((resp: NCAPIResponse) => {
        if (resp.status === 200) callback(true);
        else callback(false);
    });
}
...
export function DELETEChannel(channel_uuid: string, callback: (deleted: boolean) => void) {
    DELETE(`/Channel/${channel_uuid}`, Manager.User.token).then((resp: NCAPIResponse) => {
        if (resp.status === 200) callback(true);
        else callback(false);
    });
}
...
export function SETAvatar(user_uuid: string, file: Blob, callback: (set: boolean) => void) {
    POSTFile(`/Media/Avatar/${user_uuid}`, file, Manager.User.token).then((resp: NCAPIResponse) => {
        if (resp.status === 200) callback(true);
        else callback(false);
    });
}
...
export function SETChannelIcon(channel_uuid: string, file: Blob, callback: (set: boolean) => void) {
    POSTFile(`/Media/Channel/${channel_uuid}`, file, Manager.User.token).then((resp: NCAPIResponse) => {
        if (resp.status === 200) callback(true);
        else callback(false);
    });
}
