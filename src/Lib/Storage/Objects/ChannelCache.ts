...
import { RequestMessage, RequestMessages, RequestMessageTimestamps } from "Lib/API/Endpoints/Messages";


export class ChannelCacheResult {
...
export class ChannelCache {
  ...
  static async Open(channel_uuid: string) : Promise<ChannelCache> {
    const database = await IndexedDB.Open("Caches");
    if (!database.GetAllStores().includes(channel_uuid)) {
      return new ChannelCache(await database.CreateStore(channel_uuid) as IndexedDBStore);
    }
    return new ChannelCache(database.GetStore(channel_uuid) as IndexedDBStore);
  }
  ...
  async GetMessage(id: string) : Promise<ChannelCacheResult> {
    if (!await this.ContainsMessage(id)) return new ChannelCacheResult([], 0, 0, false);
    const message = (await this.CurrentCache.Get<IMessageProps>(id));
    return new ChannelCacheResult([message], 1, parseInt(message.message_Id), true);
  }

  async GetMessages(limit: number, before: number = 2147483647) : Promise<ChannelCacheResult> {
    ...
    perf.Stop();
    return new ChannelCacheResult(messages, messages.length, parseInt(lastID), curLim === limit)
  }

  async GetOldestMessage() : Promise<ChannelCacheResult> {
    const keys = (await this.CurrentCache.GetAllKeys()).reverse();
   ...
   */
  static async ContainsCache(channel_uuid: string) : Promise<ChannelCache | undefined> {
    const caches = await this.GetCaches();
    if (caches.indexOf(channel_uuid) !== -1) return await ChannelCache.Open(channel_uuid);
  ...
  static async CacheIsOld(channel_uuid: string) : Promise<[number, number, number] | undefined> {
    ...
    const remote_message = parseInt((await RequestMessages(channel_uuid, () => {}, true, 1))[0].message_Id);
    const local_message = parseInt((await (ce as ChannelCache).GetMessages(1)).Messages[0].message_Id);
  ...
  static async UpdateCache(channel_uuid: string) : Promise<string[] | undefined> {
    const ce = await this.ContainsCache(channel_uuid);
    if (ce === undefined) return undefined;
    const cache = ce as ChannelCache;
    ...
    const remote = await RequestMessageTimestamps(channel_uuid, limit, oldestId - 1, newestID + 1);
    ...
    for (const key in remote.keys()) {
      if ((!local.containsKey(key) || remote.getValue(key) !== local.getValue(key)) && remote.containsKey(key)) {
        const message = await RequestMessage(channel_uuid, key.toString(), true);
